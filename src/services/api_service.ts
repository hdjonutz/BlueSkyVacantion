...
export class ApiService {
    ...
    private constructor() {
        // TODO: The following code is UI related, move it out of the service into the main UI component (outside the
        // router)!
        // Global handler for api failures. We display a snack on failure to inform the user about the problems.

        this.localConfigurationService
            .get()
            .first()
            // Nicht in der KB-App
            .filter((c) => c.appType !== 'INFO')
            .subscribe(() => {
                this.statusSubject
